...
export class EnvironmentBuilder {
    ...
    updateGroundElevation(activeLevelElev = 0, minElev = 0, activeLevelConfig = null) {
        if (!this.ground || !this.grid) return;
        const isSub = (activeLevelElev < -0.1) || 
                      (activeLevelConfig?.type === 'foundation' || activeLevelConfig?.type === 'basement');
        if (isSub) {
            this.ground.position.y = activeLevelElev - 0.5;
            this.grid.position.y = activeLevelElev - 0.05;
        } else {
        ...
        if (skyConfig) {
            if (skyConfig.type === 'hdri' && skyConfig.url) {
                ...
                loader.load(skyConfig.url, (texture) => {
                    if (!this.pmremGenerator && this.ctx.renderer && typeof this.ctx.renderer.compile === 'function') {
                        try {
    ...
    buildWallGroup(w) {
        ...
        return res ? (res.wallGroup || res) : null;
    }
            ...
            try {
                ...
                if (data.rooms) {
                    ...
                    data.roomPaths.forEach(path => {
                        const floorMesh = FloorSlabEngine.buildFloorSlabMesh({ path, configId: 'hardwood', thickness: 2 }, {
                            stairsBelow,
                            shapes: data.shapes || [],
                            isSub: false,
                            subH: 2,
                ...
                if (data.walls) {
                    data.walls.forEach((w, wallIndex) => {
                        ...
                        const hasLeftTex = Boolean(w.params?.textureLeft || w.params?.textureSides || w.params?.texture);
                        const hasRightTex = Boolean(w.params?.textureRight || w.params?.textureSides || w.params?.texture);
