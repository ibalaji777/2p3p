import { GRID, PX_TO_FT, SNAP_DIST, WALL_HEIGHT, WALL_REGISTRY, WIDGET_REGISTRY, FURNITURE_REGISTRY, WORKSPACE_2D_SHAPES } from './registry.js';
import Konva from 'konva';
import { PremiumHipRoof } from './engine2d/PremiumHipRoof.js';
import { PremiumRailing } from './engine2d/PremiumRailing.js';
import { SmartGuidesTrackingSystem } from './engine2d/SmartGuidesTrackingSystem.js';
...
export class PremiumWidget {
    constructor(planner, wall, t, configId) {
        ...
        this.cutter = new Konva.Rect({ height: (wall.thickness || wall.config.thickness) + 4, fill: 'black', globalCompositeOperation: 'destination-out', listening: false }); this.planner.wallLayer.add(this.cutter);
        this.visualGroup = new Konva.Group({ draggable: false }); 
        this.hitBox = new Konva.Rect({ fill: 'transparent', listening: true }); this.visualGroup.add(this.hitBox);
        this.frameL = new Konva.Rect({ width: 4, fill: '#374151' }); this.frameR = new Konva.Rect({ width: 4, fill: '#374151' }); this.visualGroup.add(this.frameL, this.frameR);
    ...
    initEvents() {
        ...
        this.visualGroup.on('dblclick dbltap', (e) => { if (this.planner.tool === 'select') { this.facing = this.facing === 1 ? -1 : 1; this.update(); this.planner.syncAll(); e.cancelBubble = true; if(e.evt) e.evt.stopPropagation(); } });
    }
    ...
    update() {
        ...
        const hitHeight = Math.max(thick + 20, this.width * 2); this.hitBox.setAttrs({ x: -hw, y: -hitHeight / 2, width: this.width, height: hitHeight });
...
export class PremiumWall {
    ...
    initEvents() {
        ...
        this.poly.on('mousedown touchstart', (e) => {
            ...
            const toolIsWidget = !!WIDGET_REGISTRY[this.planner.tool] || isAdvancedOpening;
...
export class FloorPlanner {
    ...
    updateToolStates() {
        ...
        this.walls.forEach(w => {
            ...
            if(w.poly) {
                ...
                w.poly.setAttr('listening', canEditThisWall || (isWidget && !isRailing) || isSplitWall || isAdvancedOpening);
            }            
            w.attachedWidgets.forEach(widg => { 
                let group = widg.visualGroup || widg.group;
                if(group) { 
                    let canEditWidget = isSelect && (allowAll || cat === 'doors_windows' || cat === 'advance_openings');
        ...
        if (this.wallLayer) { this.wallLayer.listening(allowAll || cat === "common" || cat === "walls" || cat === "doors_windows" || cat === "advance_openings" || cat === "structures" || isWidget || isSplitWall || isAdvancedOpening); }
    ...
    initStageEvents() {
        ...
        this.stage.on("mousemove touchmove", () => {
            ...
            this.preview = new Konva.Line({ points: [this.lastAnchor.x, this.lastAnchor.y, snapPos.x, snapPos.y], stroke: drawColor, strokeWidth: previewThickness, lineCap: 'square', opacity: 0.6 });
        ...
        const state = {
            walls: this.walls.map(w => ({
                ...
                widgets: w.attachedWidgets.map(wid => ({
                    t: wid.t, type: wid.type, configId: wid.configId || wid.type, width: wid.width, height: wid.height, depth: wid.depth, elevation: wid.elevation,
                ...
                state.walls.forEach(wData => {
                    ...
                    if (wData.widgets) {
                        wData.widgets.forEach(wd => {
                            let widget;
                            if (['arch_opening', 'circular_opening', 'custom_shape_opening', 'niche_recess', 'pattern_opening', 'boolean_cut'].includes(wd.configId || wd.type)) {
                                widget = new advance_openings(this, wall, wd.t, wd.configId || wd.type);
                                ...
                                if (wd.rows !== undefined) widget.rows = wd.rows;
                                if (wd.cols !== undefined) widget.cols = wd.cols;
                                if (wd.spacing !== undefined) widget.spacing = wd.spacing;
                                if (wd.patternStyle !== undefined) widget.patternStyle = wd.patternStyle;
                                if (wd.decorConfigId !== undefined) widget.decorConfigId = wd.decorConfigId;
                            } else {
                                widget = new PremiumWidget(this, wall, wd.t, wd.configId || wd.type);
