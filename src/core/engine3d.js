...
// ==========================================================
// IMPORT MODERN GLTF LOADER
// ==========================================================
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';

import { WIDGET_REGISTRY, FURNITURE_REGISTRY, WALL_HEIGHT, DOOR_HEIGHT, WINDOW_SILL, WINDOW_HEIGHT } from './registry.js';
...
export class Preview3D {
    constructor(containerEl) {
        ...
        const groundMat = new THREE.MeshStandardMaterial({ color: 0x4a7c59, roughness: 1.0 }); 
        ...
        this.modelCache = {}; // Cache to store loaded GLB scenes
    ...
    getProceduralTexture(type, colorHex) {
        ...
        const texture = new THREE.CanvasTexture(canvas); texture.wrapS = THREE.RepeatWrapping; texture.wrapT = THREE.RepeatWrapping; texture.repeat.set(2, 1); texture.colorSpace = THREE.SRGBColorSpace; this.proceduralTextures[key] = texture; return texture;
    }
    ...
    createElevation3DTexture(layers, wallLen) {
        ...
        const texture = new THREE.CanvasTexture(canvas); texture.colorSpace = THREE.SRGBColorSpace; texture.wrapS = THREE.ClampToEdgeWrapping; texture.wrapT = THREE.ClampToEdgeWrapping; texture.repeat.set(1, 1); return texture;
    }
    ...
    async loadAndPlaceFurniture(furnitureEntity) {
        const config = FURNITURE_REGISTRY[furnitureEntity.config.id];
        ...
        try {
            if (!this.modelCache[config.id]) {
                ...
                // Store the loaded scene in our cache
                this.modelCache[config.id] = gltf.scene;
            }
            
            // Clone the cached object
            const objectToPlace = this.modelCache[config.id].clone();
            
            this.applyFurnitureTransforms(objectToPlace, furnitureEntity);

        } catch (error) {
            console.error(`Failed to load GLB model: ${config.model}`, error);
            this.createFallbackFurniture(furnitureEntity); 
        }
    }

    applyFurnitureTransforms(object, entity) {
        // Map 2D position (Konva) to 3D Floor (Three.js)
        object.position.set(entity.group.x(), 0, entity.group.y());
        
        // Map Rotation
        object.rotation.y = -entity.rotation * (Math.PI / 180);
        
        // Bounding Box Scaling Algorithm
        ...
        // X correlates to width, Z correlates to depth, Y is height
        const scaleX = entity.width / safeW;
        const scaleZ = entity.depth / safeD;
        const scaleY = entity.height / safeH;

        object.scale.set(scaleX, scaleY, scaleZ);
        
        // Apply realistic shadow settings to all inner meshes of the GLB
    ...
    createFallbackFurniture(entity) {
        // Creates a placeholder orange box if the .glb file is missing
        const geo = new THREE.BoxGeometry(entity.width, entity.height, entity.depth);
        const mat = new THREE.MeshStandardMaterial({ color: 0xea580c });
        const mesh = new THREE.Mesh(geo, mat);
        mesh.position.set(entity.group.x(), entity.height / 2, entity.group.y());
