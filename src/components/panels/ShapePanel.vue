<template>
    <div class="props-panel-inner">
        <div v-if="!selectedEntity.params.isEditingMaterials">
            ...
            <div class="control-group"><label>Height (Thickness)</label><div class="input-wrap"><input type="range" v-model.number="selectedEntity.params.height3D" min="10" max="1000" @input="$emit('sync-engine')"><DimensionInput v-model="selectedEntity.params.height3D" @change="$emit('sync-engine')" /></div></div>
            <div v-if="selectedEntity.type === 'shape_rect'">
                <div class="control-group"><label>Width</label><div class="input-wrap"><input type="range" v-model.number="selectedEntity.params.width" min="10" max="1000" @input="$emit('sync-engine')"><DimensionInput v-model="selectedEntity.params.width" @change="$emit('sync-engine')" /></div></div>
                <div class="control-group"><label>Depth</label><div class="input-wrap"><input type="range" v-model.number="selectedEntity.params.height" min="10" max="1000" @input="$emit('sync-engine')"><DimensionInput v-model="selectedEntity.params.height" @change="$emit('sync-engine')" /></div></div>
            </div>
