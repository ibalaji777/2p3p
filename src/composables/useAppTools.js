...
export function useAppTools({
    ...
    const setTool = (tool, params = undefined) => { 
        ...
        } else if (tool === 'elevation_segment') {
            if (!activePresetParams.value || activePresetParams.value.type !== 'elevation_segment') {
                activePresetParams.value = { type: 'elevation_segment', width: 30, depth: 40, material: 'wood', hasSpotlights: false, spotlightSpacing: 80, length: 180 };
                planner.value.activePresetParams = activePresetParams.value;
            }
